...
async def empty_query(query: InlineQuery,state: FSMContext):
    items = [ dict(name=item['name'],price=item['price'],id=item['id'],img_link=item['img_link'],flavour=item['flavour']) for item in await db.select_products_from_category(category=query.query)]
    ...
    articles = [
        ...
        description=f"Вкус: {item['flavour']}\nСтоит: 🤑 {item['price']}",
